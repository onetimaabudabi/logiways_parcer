...
# ─── Геокодирование: справочники стран и запасных координат ───────────────────
# Яндекс.Геокодер по одному названию порта часто не находит объект («Мундра»
# без страны — неоднозначно). Здесь страна проставляется принудительно.
_PORT_COUNTRY_OVERRIDE = {
    # Индия
    "Мундра": "Индия", "Нава-Шева": "Индия", "Ченнаи": "Индия",
    "Колката": "Индия", "Кочин": "Индия", "Вишакхапатнам": "Индия",
    "Тутикорин": "Индия", "Визаг": "Индия",
    # Юго-Восточная Азия
    "Коломбо": "Шри-Ланка", "Джакарта": "Индонезия", "Сурабая": "Индонезия",
    "Белаван": "Индонезия", "Пусан": "Корея", "Инчхон": "Корея",
    "Гаосюн": "Тайвань", "Тайчжун": "Тайвань", "Килунг": "Тайвань",
    "Сингапур": "Сингапур", "Порт-Кланг": "Малайзия",
    "Пасир-Гуданг": "Малайзия", "Пенанг": "Малайзия",
    "Танджунг-Пелепас": "Малайзия", "Лаем-Чабанг": "Таиланд",
    "Латкрабанг": "Таиланд", "Бангкок": "Таиланд", "Хайфон": "Вьетнам",
    "Хошимин": "Вьетнам", "Дананг": "Вьетнам", "Янгон": "Мьянма",
    "Читтагонг": "Бангладеш", "Карачи": "Пакистан", "Манила": "Филиппины",
    "Джебель-Али": "ОАЭ", "Джабаль-Али": "ОАЭ",
    # Ближний Восток, Средиземноморье
    "Амбарли": "Турция", "Гебзе": "Турция", "Мерсин": "Турция",
    "Измир": "Турция", "Стамбул": "Турция", "Александрия": "Египет",
    "Порт-Саид": "Египет", "Думьят": "Египет", "Бейрут": "Ливан",
    "Латакия": "Сирия", "Джидда": "Саудовская Аравия",
    "Барселона": "Испания", "Валенсия": "Испания",
    # Мексика
    "Мансанильо": "Мексика",
    # Россия
    "Новороссийск": "Россия", "Калининград": "Россия",
    "Санкт-Петербург": "Россия", "Владивосток": "Россия", "Находка": "Россия",
}

# Запасные координаты: используются, если геокодер не нашёл объект даже
# со страной. Значения — центры портовых зон.
_FALLBACK_COORDS = {
    # Индия
    "Мундра": {"lat": 22.8392, "lon": 69.7359},
    "Нава-Шева": {"lat": 18.9510, "lon": 72.9470},
    "Ченнаи": {"lat": 13.0827, "lon": 80.2707},
    "Колката": {"lat": 22.5726, "lon": 88.3639},
    "Кочин": {"lat": 9.9312, "lon": 76.2673},
    "Тутикорин": {"lat": 8.7642, "lon": 78.1348},
    "Вишакхапатнам": {"lat": 17.7041, "lon": 83.2973},
    # Корея
    "Пусан": {"lat": 35.1028, "lon": 129.0403},
    "Инчхон": {"lat": 37.4563, "lon": 126.7052},
    # Тайвань
    "Гаосюн": {"lat": 22.6121, "lon": 120.2972},
    "Тайчжун": {"lat": 24.2433, "lon": 120.6048},
    "Килунг": {"lat": 25.1310, "lon": 121.7403},
    # Малайзия
    "Порт-Кланг": {"lat": 2.9996, "lon": 101.3995},
    "Пасир-Гуданг": {"lat": 1.4323, "lon": 103.9204},
    # Таиланд
    "Лаем-Чабанг": {"lat": 13.0736, "lon": 100.8838},
    "Бангкок": {"lat": 13.7367, "lon": 100.5232},
    # Вьетнам
    "Хайфон": {"lat": 20.8449, "lon": 106.6881},
    "Хошимин": {"lat": 10.7758, "lon": 106.7019},
    "Дананг": {"lat": 16.0544, "lon": 108.2022},
    # Филиппины
    "Манила": {"lat": 14.5995, "lon": 120.9842},
    # ОАЭ
    "Джебель-Али": {"lat": 24.9846, "lon": 55.0112},
    # Турция
    "Амбарли": {"lat": 40.9800, "lon": 28.6840},
    "Гебзе": {"lat": 40.8030, "lon": 29.4310},
    "Мерсин": {"lat": 36.8125, "lon": 34.6414},
    "Измир": {"lat": 38.4192, "lon": 27.1287},
    # Египет
    "Александрия": {"lat": 31.2001, "lon": 29.9187},
    "Порт-Саид": {"lat": 31.2577, "lon": 32.3020},
    "Думьят": {"lat": 31.4175, "lon": 31.8144},
    # Япония
    "Иокогама": {"lat": 35.4437, "lon": 139.6380},
    "Йокогама": {"lat": 35.4437, "lon": 139.6380},
    "Нагоя": {"lat": 35.0185, "lon": 136.8655},
    "Кобе": {"lat": 34.6901, "lon": 135.1954},
    "Тояма": {"lat": 36.6958, "lon": 137.2137},
    # Россия
    "Новороссийск": {"lat": 44.7235, "lon": 37.7677},
    "Калининград": {"lat": 54.7065, "lon": 20.5100},
    "Санкт-Петербург": {"lat": 59.9343, "lon": 30.3351},
    "Владивосток": {"lat": 43.1150, "lon": 131.8850},
    "Находка": {"lat": 42.8160, "lon": 132.8730},
}


# Добавьте этот словарь после импортов, например, после строки с импортами.
_PORT_COUNTRY_OVERRIDE = {
    "Мундра": "Индия",
    "Нава-Шева": "Индия",
    "Ченнаи": "Индия",
...
class LogiwaysClient:
    ...
    def _handle_401(self, func, *args, **kwargs):
        ...
        if not retry_allowed or self._auth_retry_active:
            print("Повторная авторизация уже выполнялась — прекращаем "
                  ...
                  "Проверьте AUTH_HEADER_SCHEME и права учётной записи.")
            return None

        renewed = self.refresh_token()
        if not renewed:
            ...
            if not renewed:
                print("Повторный логин не удался. Прекращаем.")
                return None

        self._auth_retry_active = True
        try:
    ...
    def refresh_token(self) -> Optional[Tokens]:
        ...
        try:
            ...
            print(f"Ошибка соединения при обновлении токена: {type(e).__name__}: {e}")
            return None
    ...
    def geocode_location(
        ...
        try:
            ...
            # ─── ПРИНУДИТЕЛЬНАЯ ПОДСТАНОВКА СТРАНЫ ───
            # ВАЖНО: страну определяем ДО вычисления country_name.
            # Раньше country_name считался выше этого блока, поэтому
            # подставленная страна в запрос не попадала — и «Мундра»
            # уходила в геокодер без страны, как и до подстановки.
            from shared import port_dict, get_country

            # 1) явный словарь переопределения — самый надёжный источник
            if not country_code and location_name in _PORT_COUNTRY_OVERRIDE:
                country_code = _PORT_COUNTRY_OVERRIDE[location_name]
                print(f"  [COUNTRY] Определена страна из переопределения: "
                      f"{country_code} для {location_name}")

            # 2) затем справочник shared
            if not country_code and location_name in port_dict:
                country_code = get_country(location_name)
                if country_code:
                    print(f"  [COUNTRY] Определена страна из shared.get_country: "
                          f"{country_code} для {location_name}")

            # 3) и по русскому названию порта: в port_dict ключи английские,
            #    а сюда приходит уже переведённое имя.
            if not country_code:
                found = get_country(location_name)
                if found:
                    country_code = found
                    print(f"  [COUNTRY] Страна по названию локации: "
                          f"{country_code} для {location_name}")

            # Преобразуем ISO2 код страны в название для Yandex.
            # Если пришло русское название («Индия»), оно вернётся как есть.
            ...
            # Если геокодер не нашёл объект даже со страной — берём
            # заранее заданные координаты, чтобы сегмент не остался без точки.
            if not result and location_name in _FALLBACK_COORDS:
                coords = _FALLBACK_COORDS[location_name]
                print(f"  [FALLBACK] Использованы заранее заданные координаты "
                      f"для {location_name}: {coords['lat']}, {coords['lon']}")
                return RouteSegmentCoordinate(
                    latitude=coords["lat"],
                    longitude=coords["lon"],
                    order_position=0,
                    description=f"Fallback: {location_name}",
                )
...
if __name__ == "__main__":
    ...
    # Раньше здесь стоял безусловный вызов с exit(0): скрипт заполнял
    # координаты и завершался, не доходя до загрузки компаний и тарифов.
    # Теперь геокодирование включается флагом --geocode:
    #     python3 send_api_New.py --geocode        только координаты
    #     python3 send_api_New.py                  обычная загрузка
